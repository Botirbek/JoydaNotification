...
import com.example.joydanotification.v1.repository.NotificationRepository1;
...
public class NotificationService1 {
    ...
    private final NotificationRepository1 notificationRepository;
    ...
    public ResponseEntity<DataDTO<Long>> save( NotificationCreateDTO notificationCreateDTO, Long userId) {
    ...
    public ResponseEntity<DataDTO<NotificationResponseDTO>> getCountNewNotification( String lan,Long userId) {
        List<Notification> notifications = notificationRepository.countAllByReadStatusAndUserId(userId);
        List<NotificationItemDTO> dataList = getDataList(notifications, lan);
        int pageCount =(int) Math.ceil((double) dataList.size()/30);
        return ResponseEntity.ok(new DataDTO(new NotificationResponseDTO(dataList,pageCount)));
    }
